...
@property (nonatomic, strong) AVMutableComposition *composition;  //可变文件 合并音视频成分
...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)(NSError *error))handler progress:(void (^)(float progress))progress {
    ...
    if (orientation != UIImageOrientationUp || self.graffitiView) {
        ...
        if(self.graffitiView || self.stickerLayers.count) {
            //涂鸦层 生成图片
            CALayer *layer = [self graffitiViewLayer:renderSize];
            //贴画层
